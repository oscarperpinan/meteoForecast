getRaster <- function(var='swflx',
    ...
    service <- match.arg(service, c('meteogalicia', 'gfs', 'openmeteo'))
    ## Model initialization time
    run <- switch(service,
                  meteogalicia = match.arg(run, c('00', '12')),
                  gfs = match.arg(run, c('00', '06', '12', '18')),
    ...
    frames <-  switch(service,
                      ...
                      gfs = {
                          ## Important: GFS first frame is 0 hours since RUN
                          if (frames == 'complete') frames <- seq(0, 192, by = 3)
                          else frames <- seq(0, length = min(65, as.integer(frames)), by = 3)
                      },
                      meteogalicia = {
                          ## MeteoGalicia implements netCDF Time Subset. Therefore, `frames` is a chara
                          if (remote) {
    ...
    ncFile <- switch(service,
                     meteogalicia = paste0(paste(var, dd, run, 
                         sep='_'), '.nc'),
                     gfs = paste0(paste(var, dd, run, frames,
                         sep='_'), '.nc'), 
    ...
    hours <- switch(service,
                    ...
                    openmeteo = seq_len(nlayers(b))* 3600,
                    gfs = frames * 3600
                    )
